...
import kotlinx.android.synthetic.main.novel_item_bookmark.view.tv_chapter_name
import kotlinx.android.synthetic.main.novel_item_chapter_list.view.*
import org.jetbrains.anko.sdk27.listeners.onClick

class ChapterListAdapter(context: Context, val callback: Callback) :
    SimpleRecyclerAdapter<BookChapter>(context, R.layout.novel_item_chapter_list) {
    ...
    private fun upHasCache(itemView: View, isDur: Boolean, cached: Boolean) = itemView.apply {
        ...
        iv_checked.setImageResource(R.drawable.novel_ic_outline_cloud_24)
        iv_checked.visible(!cached)
        if (isDur) {
            iv_checked.setImageResource(R.drawable.novel_ic_check)
            iv_checked.visible()
        }
    }
