...
import kotlinx.android.synthetic.main.novel_dialog_read_aloud.*
...
class ReadAloudDialog : BaseDialogFragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.novel_dialog_read_aloud, container)
    }
    ...
    private fun upPlayState() {
        if (!BaseReadAloudService.pause) {
            iv_play_pause.setImageResource(R.drawable.novel_ic_pause_24dp)
        } else {
            iv_play_pause.setImageResource(R.drawable.novel_ic_play_24dp)
        }
        val bg = requireContext().bottomBackground
        val isLight = ColorUtils.isColorLight(bg)
        val textColor = requireContext().getPrimaryTextColor(isLight)
