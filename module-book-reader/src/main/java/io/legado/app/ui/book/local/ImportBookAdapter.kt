...
import kotlinx.android.synthetic.main.novel_item_import_book.view.*
import org.jetbrains.anko.sdk27.listeners.onClick


class ImportBookAdapter(context: Context, val callBack: CallBack) :
    SimpleRecyclerAdapter<DocItem>(context, R.layout.novel_item_import_book) {
    ...
    override fun convert(holder: ItemViewHolder, item: DocItem, payloads: MutableList<Any>) {
        holder.itemView.apply {
            if (payloads.isEmpty()) {
                if (item.isDir) {
                    iv_icon.setImageResource(R.drawable.novel_ic_folder)
                    iv_icon.visible()
                    cb_select.invisible()
                    ll_brief.gone()
                    ...
                    if (bookshelf.contains(item.uri.toString())) {
                        iv_icon.setImageResource(R.drawable.novel_ic_book_has)
                        iv_icon.visible()
                        cb_select.invisible()
                    } else {
                        iv_icon.invisible()
