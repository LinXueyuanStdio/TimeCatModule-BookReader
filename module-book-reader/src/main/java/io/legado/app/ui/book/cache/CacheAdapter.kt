...
import kotlinx.android.synthetic.main.novel_item_download.view.*
...
class CacheAdapter(context: Context, private val callBack: CallBack) :
    SimpleRecyclerAdapter<Book>(context, R.layout.novel_item_download) {
    ...
    private fun upDownloadIv(iv: ImageView, book: Book) {
        downloadMap?.let {
            if (it.containsKey(book.bookUrl)) {
                iv.setImageResource(R.drawable.novel_ic_stop_black_24dp)
            } else {
                iv.setImageResource(R.drawable.novel_ic_play_24dp)
            }
        } ?: let {
            iv.setImageResource(R.drawable.novel_ic_play_24dp)
        }
