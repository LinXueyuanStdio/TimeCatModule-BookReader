...
import kotlinx.android.synthetic.main.novel_item_fillet_text.view.*
import kotlinx.android.synthetic.main.novel_item_find_book.view.*
...
class ExploreAdapter(context: Context, private val scope: CoroutineScope, val callBack: CallBack) :
    SimpleRecyclerAdapter<BookSource>(context, R.layout.novel_item_find_book) {
    ...
    override fun convert(holder: ItemViewHolder, item: BookSource, payloads: MutableList<Any>) {
        with(holder.itemView) {
            ...
            if (exIndex == holder.layoutPosition) {
                iv_status.setImageResource(R.drawable.novel_ic_arrow_down)
                    ...
                    if (!kindList.isNullOrEmpty()) {
                        ...
                        kindList.map { kind ->
                            val tv = LayoutInflater.from(context)
                                .inflate(R.layout.novel_item_fillet_text, gl_child, false)
                            gl_child.addView(tv)
                ...
                iv_status.setImageResource(R.drawable.novel_ic_arrow_right)
                rotate_loading.hide()
                gl_child.gone()
            }
        }
    ...
    private fun showMenu(view: View, position: Int): Boolean {
        ...
        popupMenu.inflate(R.menu.novel_explore_item)
